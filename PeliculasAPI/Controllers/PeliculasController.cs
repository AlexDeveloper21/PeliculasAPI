...
namespace PeliculasAPI.Controllers
{
    ...
    public class PeliculasController : CustomBaseController
    {
        ...
        private string contenedor = "peliculas";
        public PeliculasController(ApplicationDbContext context, IMapper mapper, IOutputCacheStore outputCacheStore,
        ...
        public async Task<ActionResult<LandingPageDTO>> Get()
        {
            ...
            var enCines = await context.Peliculas
                .Where(p => p.FechaLanzamiento <= hoy && p.PeliculasCines.Any(pc => pc.PeliculaId == p.Id))
                .OrderBy(p => p.FechaLanzamiento)
                .Take(top)
                .ProjectTo<PeliculaDTO>(mapper.ConfigurationProvider)
                .ToListAsync();
        ...
        public async Task<ActionResult<List<PeliculaDTO>>> Filtrar([FromQuery] PeliculasFiltrarDTO peliculasFiltrarDTO)
        {
            ...
            {
                peliculasQueryable = peliculasQueryable.Where(p => 
                    p.PeliculasGeneros.Select(pg => pg.GeneroId).Contains(peliculasFiltrarDTO.GeneroId)
                );
            }
        ...
        public async Task<IActionResult> Put(int id, [FromForm] PeliculaCreacionDTO peliculaCreacionDTO)
        {
            ...
            if (peliculaCreacionDTO.Poster is not null)
            {
                pelicula.Poster = await almacenadorArchivos
                    .Editar(pelicula.Poster, contenedor, peliculaCreacionDTO.Poster);
            }
